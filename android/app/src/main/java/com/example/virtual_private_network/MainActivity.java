package com.example.virtual_private_network;
...
public class MainActivity extends FlutterActivity {
                ...
                switch (call.method) {
                    case "stop":
                        try {
                            ...
                            setStage("disconnected");
                            result.success(null); // Still consider it successful
                        }
                        break;
                    case "start":
                        ...
                        Log.d(TAG, "=== VPN START REQUEST ===");
                        Log.d(TAG, "Country: " + name);
                        Log.d(TAG, "Username: " + username);
                        Log.d(TAG, "Password: " + password);
                        Log.d(TAG, "Config length: " + (config != null ? config.length() : 0));
                        Log.d(TAG, "Config preview: " + (config != null ? config.substring(0, Math.min(200, config.length())) : "null"));

                        if (config == null || name == null) {
                            Log.e(TAG, "Config not valid!");
                            result.error("INVALID_CONFIG", "Config not valid", null);
                            return;
                        }
    ...
    private void prepareVPN() {
        if (isConnected()) {
            ...
            try {
                ...
                if (vpnProfile == null) {
                    Log.e(TAG, "Failed to convert VPN profile");
                    setStage("disconnected");
                    return;
                }
                
            } catch (IOException e) {
                Log.e(TAG, "IOException in prepareVPN: " + e.getMessage());
                setStage("disconnected");
                e.printStackTrace();
                return;
            } catch (ConfigParser.ConfigParseError configParseError) {
                Log.e(TAG, "ConfigParseError in prepareVPN: " + configParseError.getMessage());
                setStage("disconnected");
                configParseError.printStackTrace();
                return;
            } catch (Exception e) {
                Log.e(TAG, "Exception in prepareVPN: " + e.getMessage());
                setStage("disconnected");
                e.printStackTrace();
                return;
            }
    ...
    private void startVPN() {
        try {
            ...
            Log.e(TAG, "Exception in startVPN: " + e.getMessage());
            setStage("disconnected");
            e.printStackTrace();
        }
    }
